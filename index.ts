...
export namespace RankChatMain {
    ...
    export function getPlayerChat(player: ServerPlayer, chat: string): string {
        const rank = RankPerms.getDisplay(PlayerRank.getRank(player))+"§r" ?? PlayerRank.getRank(player);
        const username = player.getName();
        const message = chat.replace(/§/g, "");
        let msg = getRankChat(PlayerRank.getRank(player));
    ...
    export function ChatPlayer(player: ServerPlayer|undefined, target: ServerPlayer, message: string): void {
        if (player) {
            ...
            const rank1 = RankPerms.getDisplay(PlayerRank.getRank(player))+"§r" ?? PlayerRank.getRank(player);
            const rank2 = RankPerms.getDisplay(PlayerRank.getRank(target))+"§r" ?? PlayerRank.getRank(target);
            ...
            const rank1 = RankPerms.getRanks()[0];
            const rank2 = PlayerRank.getRank(target);
            const display1 = RankPerms.getDisplay(rank1)+"§r" ?? rank1;
            const display2 = RankPerms.getDisplay(rank2)+"§r" ?? rank2;
...
events.packetSend(MinecraftPacketIds.Text).on((pkt, netId) => {
    ...
    const rank = RankPerms.getDisplay(PlayerRank.getRank(player))+"§r" ?? PlayerRank.getRank(player);
    let msg = RankChatMain.getRankChat(PlayerRank.getRank(player));
